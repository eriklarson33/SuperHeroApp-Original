...
var orgId = String($('#add-id').val());
//alert("The current Site Id is: " + orgId);
var chkArray = [];
...
$('#updateSupersPresent').on("click", function () {
    ...
    if (typeof chkArray !== 'undefined' && chkArray.length > 0) {
        ...
        $.ajax({
            type: 'PUT',
            url: 'http://localhost:8080/HEROdb/superHumanXorganizations/',
            data: JSON.stringify({
                orgSupers: chkArray,
        ...
        $.ajax({
            type: 'DELETE',
            url: 'http://localhost:8080/HEROdb/organization/' + orgId,
            success: function () {
                // clear errorMessages
                $('#errorMessages').empty();
                succesfulAJAX();
